...
execute as @e[limit=1,distance=..10,sort=random,tag=!global.ignore,tag=!pht.snared,type=bee] at @s if block ~ ~-1 ~ #pht:corpsebloom_valid_blocks run function pht:entity/corpsebloom/grown/snare
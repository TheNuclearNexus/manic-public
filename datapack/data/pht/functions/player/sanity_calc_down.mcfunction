...
execute unless entity @e[distance=..16,type=#pht:sanity_increase] if predicate pht:in_the_end run scoreboard players remove @s pht.temp_sanity 1
...
execute if predicate pht:in_overworld if predicate pht:new_moon unless predicate pht:low_height run scoreboard players remove @s pht.temp_sanity 3
...
execute if score @s pht.dummy matches 0 run scoreboard players remove @s pht.temp_sanity 4
